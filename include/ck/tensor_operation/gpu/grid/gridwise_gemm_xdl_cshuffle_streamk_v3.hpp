...
struct GridwiseGemm_xdl_cshuffle_v3
{
    ...
    __device__ static void Run(const ADataType* p_a_grid,
                               ...
                               Problem& problem)
    {
        // auto splitk_batch_offset = SplitKBatchOffset(problem, 0);

        // const auto a_grid_desc_ak0_m_ak1 = MakeAGridDescriptor_AK0_M_AK1(
        //     problem.M, problem.MPadded, problem.K, problem.KPadded, problem.StrideA,
        //     problem.AK0);
        // const auto b_grid_desc_bk0_n_bk1 = MakeBGridDescriptor_BK0_N_BK1(
        //     problem.K, problem.KPadded, problem.N, problem.NPadded, problem.StrideB,
        //     problem.BK0);
        ...
        // printf(" get_block_1d_id()=%0d, blockidx.x=%0d,blockidx.z=%0d\n",
        //        get_block_1d_id(),
        //        blockIdx.x,
        //        blockIdx.z);
        auto orig_K = problem.K;
        for(auto block_idx = get_block_1d_id();
            block_idx < block_2_ctile_map.CalculateGridSize(problem.M, problem.N);
            block_idx += gridDim.x)
        {
            for(unsigned int kbatch_id = 0; kbatch_id < static_cast<unsigned int>(problem.KBatch);
                kbatch_id++)
            {
                auto splitk_batch_offset = SplitKBatchOffset(problem, kbatch_id, orig_K);
                                    ...
                                    Problem& problem)
    {
        ...
        // divide block work by [M, N]
        const auto block_2_ctile_map = Block2CTileMap{problem.M, problem.N, 4};
        //   printf(" get_block_1d_id()=%0d, blockidx.x=%0d,
        //   blockidx.z=%0d\n",get_block_1d_id(),blockIdx.x, blockIdx.z);
        auto orig_K = problem.K;
        for(auto block_idx = get_block_1d_id();
            block_idx < block_2_ctile_map.CalculateGridSize(problem.M, problem.N);
            block_idx += gridDim.x)
